...
public class ThreadTest2 extends Thread {
	...
	Thread thread3 = new Thread() {
		public void run() {
			...
			for (int h = 0; h<thirdHalf.size();h++) {
				//System.out.println(firstHalf.get(h));
				BasicDBObject whereQuery = new BasicDBObject();
				whereQuery.put("UnitId", thirdHalf.get(h));
				...
				for (int m = 0; m < arraylist.size(); m++) {
					...
					obj.add(new BasicDBObject("UnitId", thirdHalf.get(h)));
					...
					String json = "{\"UnitId\": " + "\"" + thirdHalf.get(h) + "\"" + ",\"Dag\": " + "\"" + arraylist.get(m) + "\""
							+ ", \"KM\": " + "\"" + Math.round(gereden) + "\"}";

					DBObject dbObject = (DBObject) JSON.parse(json);
					collUnits.insert(dbObject);

					 System.out.println("{\"UnitId\": " + "\"" + thirdHalf.get(h) + "\"" + ",\"Dag\": " + "\"" + arraylist.get(m) + "\""
					 + ", \"KM\": " + "\"" + Math.round(gereden) + "\"}");

				}
				System.out.println(
						"UnitId: " + thirdHalf.get(h) + ", Totaal KM vanaf " + arraylist.get(0) + ": " + Math.round(geredenTotaal));
				System.out.println(
						"UnitId: " + thirdHalf.get(h) + ", Gemiddeld KM per dag: " + Math.round(geredenTotaal / arraylist.size()));
