...
public class Wagenparkbeheer {
	...
	public void test() {
		...
		for (DBObject result : output.results()) {
			...
			//
			// System.out.println(
			// "UnitId: " + unit + ", Vanaf " + arraylist.get(0) + ": " +
			// Math.round(geredenTotaal));
			// System.out.println(
			// "UnitId: " + unit + ", Gemiddeld KM per dag: " +
			// Math.round(geredenTotaal / arraylist.size()));
...
// BasicDBObject index = new BasicDBObject("Rdxy", 1);
// com.createIndex(index);
// DBCursor cursor = com.find().sort(index).limit(1000);
// while (cursor.hasNext()) {
// String rdxy = cursor.next().get("Rdxy").toString();
//
// if (comHash.containsKey(rdxy)) {
// comHash.put(rdxy, comHash.get(rdxy)+1);
// // System.out.println(comHash);
// } else {
// comHash.put(rdxy, 1);
// //System.out.println(comHash);
// //System.out.println(comHash.get(cursor.next().get("Rdxy")));
// }
// }
// System.out.println("Size of Map: " + comHash.size());
//
// }
//
// }