...
public class ThreadTest extends Thread {
	...
	Thread thread1 = new Thread() {
		public void run() {
			...
			for (int h = 0; h<firstHalf.size();h++) {
				//System.out.println(firstHalf.get(h));
				BasicDBObject whereQuery = new BasicDBObject();
				whereQuery.put("UnitId", firstHalf.get(h));
				...
				for (int m = 0; m < arraylist.size(); m++) {
					...
					obj.add(new BasicDBObject("UnitId", firstHalf.get(h)));
					...
					String json = "{\"UnitId\": " + "\"" + firstHalf.get(h) + "\"" + ",\"Dag\": " + "\"" + arraylist.get(m) + "\""
							+ ", \"KM\": " + "\"" + Math.round(gereden) + "\"}";

					DBObject dbObject = (DBObject) JSON.parse(json);
					collUnits.insert(dbObject);

					 System.out.println("{\"UnitId\": " + "\"" + firstHalf.get(h) + "\"" + ",\"Dag\": " + "\"" + arraylist.get(m) + "\""
					 + ", \"KM\": " + "\"" + Math.round(gereden) + "\"}");

				}
				System.out.println(
						"UnitId: " + firstHalf.get(h) + ", Totaal KM vanaf " + arraylist.get(0) + ": " + Math.round(geredenTotaal));
				System.out.println(
						"UnitId: " + firstHalf.get(h) + ", Gemiddeld KM per dag: " + Math.round(geredenTotaal / arraylist.size()));
