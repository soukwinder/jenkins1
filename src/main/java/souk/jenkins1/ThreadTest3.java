...
public class ThreadTest3 extends Thread {
	...
	ArrayList<String> thirdHalf = new ArrayList<String>();
	ArrayList<String> fourthHalf = new ArrayList<String>();
	...
	Thread thread4 = new Thread() {
		public void run() {
			...
			for (int h = 0; h<fourthHalf.size();h++) {
				//System.out.println(firstHalf.get(h));
				BasicDBObject whereQuery = new BasicDBObject();
				whereQuery.put("UnitId", fourthHalf.get(h));
				...
				for (int m = 0; m < arraylist.size(); m++) {
					...
					obj.add(new BasicDBObject("UnitId", fourthHalf.get(h)));
					...
					String json = "{\"UnitId\": " + "\"" + fourthHalf.get(h) + "\"" + ",\"Dag\": " + "\"" + arraylist.get(m) + "\""
							+ ", \"KM\": " + "\"" + Math.round(gereden) + "\"}";

					DBObject dbObject = (DBObject) JSON.parse(json);
					collUnits.insert(dbObject);

					 System.out.println("{\"UnitId\": " + "\"" + fourthHalf.get(h) + "\"" + ",\"Dag\": " + "\"" + arraylist.get(m) + "\""
					 + ", \"KM\": " + "\"" + Math.round(gereden) + "\"}");

				}
				System.out.println(
						"UnitId: " + fourthHalf.get(h) + ", Totaal KM vanaf " + arraylist.get(0) + ": " + Math.round(geredenTotaal));
				System.out.println(
						"UnitId: " + fourthHalf.get(h) + ", Gemiddeld KM per dag: " + Math.round(geredenTotaal / arraylist.size()));
